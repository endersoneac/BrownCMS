<?php
class CorePDOException extends ErrorException {}

class CorePDO {
	
	public static $sgbd = "mysql";
	public static $dbhost = "localhost";
	public static $dbname = "BrownCMS";
	public static $dbuser = "brown";
	public static $dbpass = "sodkaos";
	...
	public static function connect() {
		try {
			...
			throw new CorePDOException ( $ex->getMessage () );
		}
	}
	...
	public static function update($table, $columns, $values, $where = NULL) {
		...
		if (is_array ( $columns ) and is_array ( $values )) {
			if ((count ( $columns )) == (count ( $values ))) {
				...
				throw new CorePDOException ( "We must have the same number of columns and values." );
			}
		} else if (is_array ( $columns ) and ! is_array ( $values )) {
			throw new CorePDOException ( "'values' must be an array." );
		} 

		else if (! is_array ( $columns ) and is_array ( $values )) {
			throw new CorePDOException ( "'columns' must be an array." );
		} 
			...
			if ($conn = self::connect ()) { // se conectar
				...
				if (! $result) {
					throw new CorePDOException ( $stmt->errorInfo () );
				}
				
				$conn = null;
				return true;
			} else {
				throw new CorePDOException ( "Unable to connect to database!<br>\n<i>Check the connection variables." );
			}
		}
		if ($sucess == true) {
			if ($conn = self::connect ()) { // se conectar
				...
				throw new CorePDOException ( "Unable to connect to database! Check the connection variables." );
			}
		}
	}
	...
	public static function consult($table, $columns = "*", $where = NULL, $order = NULL, $limit = NULL, $like = NULL) {
		...
		if ($table) {
			...
			throw new CorePDOException ( "No table has been indicated." );
		}
		if ($columns) {
		...
		if ($order) {
			...
			if ($c == "+") {
				...
				throw new CorePDOException ( "Tell +/- at the end of variable order!" );
			}
		} else {
			$order = NULL;
		}
		if ($limit) {
			if (is_numeric ( $limit )) {
				...
				throw new CorePDOException ( "Enter a numeric limit!" );
			}
		} else {
			$limit = NULL;
		}
		...
		if ($conn = self::connect ()) {
			if ($result = $conn->query ( $sql )) {
				...
				if (empty ( $rows )) {
					throw new CorePDOException ( "No results!" );
				}
				$conn = null;
				return $rows;
			} else {
		...
		if ($conn = self::connect ()) {
			if ($result = $conn->query ( $sql )) {
				...
				if ($result = $stmt->execute ()) {
					...
					throw new CorePDOException ( "Invalid query!" );
				}
				$conn = null;
				return true;
			} else {
			...
			if (stripos ( $where, "where" ) !== false) {
				if (! strstr ( $where, "'" )) {
					...
					for($j = 0; $j < count ( $arr1 ); $j ++) {
						if (stripos ( $arr1 [$j], "=" ) !== false || stripos ( $arr1 [$j], "!" ) !== false || stripos ( $arr1 [$j], "<" ) !== false || stripos ( $arr1 [$j], ">" ) !== false) {
							$arr1 [$j + 1] = "'" . $arr1 [$j + 1] . "'";
						}
						$newwhere .= " " . $arr1 [$j] . " ";
					}
